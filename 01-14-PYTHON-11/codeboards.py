...
'''
Задание 4.4
Подготовка данных о вакансиях с платформы hh.ru. В распоряжении имеется таблица,
в которой с помощью парсинга собраны резюме кандидатов. В этой таблице есть
текстовый столбец «Опыт работы». Пример такого столбца представлен ниже в виде
объекта Series. Структура текста в столбце фиксирована и не может измениться.

Написать функцию get_experience(arg), аргументом которой является строка столбца
с опытом работы. Функция должна возвращать опыт работы в месяцах. Не забыть
...
def get_experience(arg):
    arg_list = arg.split()
    if len(arg_list) == 6:
        period = int(arg_list[2]) * 12 + int(arg_list[4])
    else:
        if arg_list[3][0] == 'м':
            period = int(arg_list[2])
        else:
            period = int(arg_list[2]) * 12
    return period
